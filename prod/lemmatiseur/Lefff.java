...
public class Lefff {
	 ...
	 * @param path chemin d'accÈs du texte ‡ traiter
	 */
	...
	* @detail Permet de lire un fichier de lefff format√© sous la forme : "abaiss√© v abaisser" afin de pr√©parer la mise √† l'infinitif des verbes.
	**/
	...
	* @param String txt nouveau texte √† √©crire
	* @param String titre titre du fichier cr√©√©e
	**/
	...
	* @detail traite le texte ce trouvant √† l'adresse envoy√© en param√®tre
	* @see writeFile(String txt, String titre)
	* @param String path chemin d'acc√©s du fichier √† traiter
	**/
	public String traiteVerbe(String oldTexte){
		...
		for(int i = 0; i < split.length; i++){
			...
			//cas particulier - aprËs le verbe
			...
			//VÈrifie qu'il y est un infinitif
			if(infinitif != "-1"){
				outils = arbreOutil.findTokem(split[i-1]);
				//VÈrifie qu'il n'est pas prÈcÈdÈ d'un article indÈfinie
				if(!outils.equals("ad")){
	...
	/**
	 * @brief vÈrifie si il y a des expressions ‡ ne pas traiter pour la lemmatisationd es verbes
	...
	public String traiteTexte(String p){
		...
		txt = traiteVerbe(txt);
		System.out.println("Traitement des expressions figÈes neutres");
	...
	/**
	 * @brief enlËve toute les expressions neutres du texte
